    ...
    <form method="POST" wire:submit.prevent="store">
        ...
        <div class="p-5 flex flex-col gap-4">
            ...
            <div class="block mb-5">
                ...
                <select id="level_id" class="block mt-1 rounded-md border-gray-300 w-full" wire:model="level_id"
                    name="level_id">
                    <option value="">Sélectionner un niveau</option>
                    @forelse ($currentLevels as $item)
                        <option value="{{ $item->id }}">{{ $item->libelle }}</option>
                    @empty
                        <option value="" disabled>Aucun niveau disponible pour l’année scolaire active</option>
                    @endforelse
                </select>
                @error('level_id')
            ...
            <div class="block mb-5">
                <x-jet-label value="{{ __('Libelle') }}" />
                <input type="text" class="block mt-1 rounded-md border-gray-300 w-full @error('code')
                    border-red-500 bg-red-100 animate-bounce
                @enderror" wire:model="libelle" name="libelle">
